    ...
    <div class="py-12 bg-slate-50 min-h-screen">
        <div class="max-w-4xl mx-auto px-6 lg:px-8">
            ...
            <div class="mb-10">
                ...
                <h1 class="font-lexend text-4xl font-bold text-stitch-primary">Editar Desafío</h1>
                <p class="text-stitch-on-surface-variant mt-2">Actualiza los detalles del reto para tus alumnos.</p>
            </div>

            <form action="{{ route('challenges.update', $challenge) }}" method="POST" class="space-y-8 bg-white p-10 md:p-12 rounded-[3rem] shadow-xl shadow-slate-200/50 border border-slate-100">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select name="career_id" id="career_id" required
                            ...
                            @foreach($careers as $career)
                                <option value="{{ $career->id }}" {{ old('career_id', $challenge->career_id) == $career->id ? 'selected' : '' }}>{{ $career->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <!-- Empresa (Opcional) -->
                    <div>
                        <label for="company_id" class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-3">Empresa Patrocinadora</label>
                        <select name="company_id" id="company_id"
                            class="w-full bg-slate-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-stitch-primary/20 text-stitch-primary font-bold">
                            <option value="">Ninguna</option>
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ old('company_id', $challenge->company_id) == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select name="difficulty" id="difficulty" required
                            class="w-full bg-slate-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-stitch-primary/20 text-stitch-primary font-bold">
                            <option value="Fácil" {{ $challenge->difficulty == 'Fácil' ? 'selected' : '' }}>Fácil</option>
                            <option value="Intermedio" {{ $challenge->difficulty == 'Intermedio' ? 'selected' : '' }}>Intermedio</option>
                            <option value="Avanzado" {{ $challenge->difficulty == 'Avanzado' ? 'selected' : '' }}>Avanzado</option>
                        </select>
                    </div>

                    <!-- Fecha Límite -->
                    <div>
                        <label for="expires_at" class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-3">Fecha de Vencimiento</label>
                        <input type="date" name="expires_at" id="expires_at" value="{{ old('expires_at', $challenge->expires_at ? $challenge->expires_at->format('Y-m-d') : '') }}"
                            class="w-full bg-slate-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-stitch-primary/20 text-stitch-primary font-bold">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="prose max-w-none">
                        <input id="description" type="hidden" name="description" value="{{ old('description', $challenge->description) }}">
