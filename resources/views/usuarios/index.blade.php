        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div class="mb-8 p-10 rounded-[2rem] bg-stitch-primary text-white relative overflow-hidden shadow-2xl">
                <div class="relative z-10">
                    ...
                    <p class="text-lg text-white/80 mt-2 max-w-2xl">Administra las cuentas, roles y estados de todos los
                        miembros de la plataforma.</p>
                </div>
            ...
            @if(session('success'))
                <div
                    class="mb-6 flex items-center gap-3 p-4 bg-green-50 border border-green-200 text-green-800 rounded-stitch text-sm font-semibold">
            ...
            <form method="GET" action="{{ route('usuarios.index') }}" class="mb-6">
                <div class="relative max-w-lg">
                    <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-stitch-on-surface-variant"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <input id="search-users" type="text" name="search" value="{{ $search ?? '' }}"
                    ...
                    @if($search ?? false)
                        <a href="{{ route('usuarios.index') }}"
                            class="absolute right-4 top-1/2 -translate-y-1/2 text-stitch-on-surface-variant hover:text-stitch-primary text-xs font-bold">✕
                            Limpiar</a>
                    @endif
                </div>
            </form>
            ...
            @if($search ?? false)
                <p class="mb-4 text-sm text-stitch-on-surface-variant">
                    Mostrando <strong>{{ $users->count() }}</strong> resultado(s) para "<span
                        class="text-stitch-primary font-semibold">{{ $search }}</span>"
                </p>
            @endif
            ...
            <div class="bg-white rounded-stitch shadow-sm border border-stitch-outline/10 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                class="bg-stitch-background text-stitch-primary font-lexend text-sm uppercase tracking-wider border-b border-stitch-outline/10">
                        ...
                        <tbody class="divide-y divide-stitch-outline/10">
                            @forelse($users as $user)
                                <tr
                                    class="hover:bg-stitch-background/50 transition-colors {{ $user->is_suspended ? 'opacity-60' : '' }}">
                                    <td class="p-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-10 h-10 rounded-full {{ $user->is_suspended ? 'bg-gray-200 text-gray-400' : 'bg-stitch-secondary/10 text-stitch-secondary' }} flex items-center justify-center font-bold font-lexend flex-shrink-0">
                                    ...
                                    <td class="p-4">
                                        ...
                                        @if($user->user_type === 'maestro')
                                            <span
                                                class="px-3 py-1 bg-[#F3E5F5] text-[#8E24AA] rounded-full text-xs font-bold uppercase tracking-wider">Mentor</span>
                                        @elseif($user->user_type === 'administrador')
                                            <span
                                                class="px-3 py-1 bg-[#FFF3E0] text-[#E65100] rounded-full text-xs font-bold uppercase tracking-wider">Admin</span>
                                        @else
                                            <span
                                                class="px-3 py-1 bg-[#E3F2FD] text-[#1976D2] rounded-full text-xs font-bold uppercase tracking-wider">Estudiante</span>
                                        @endif
                                    </td>
                                    <td class="p-4">
                                        @if($user->is_suspended)
                                            <span
                                                class="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">Suspendido</span>
                                        @else
                                            <span
                                                class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">Activo</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="p-4">
                                        <div class="flex items-center justify-end gap-2">
                                            {{-- Change Role form --}}
                                            @if(auth()->id() !== $user->id)
                                                <form method="POST" action="{{ route('usuarios.changeRole', $user) }}"
